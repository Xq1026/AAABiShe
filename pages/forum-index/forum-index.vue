<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" @scroll="scrollChange">
		...
		<view :style='{"width":"100%","padding":"20rpx 10rpx 70rpx","background":"#f3f5f9"}'>
			<!--header end-->
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"none","display":"flex","height":"auto"}'>
				<view class="search-form round" :style='{"margin":"0 12rpx 0 0","flex":"1","position":"relative"}'>
					...
					<input :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"60rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.title" type="text" placeholder="标题" ></input>
				</view>
			...
			<button :style='{"padding":"0 40rpx","margin":"40rpx auto 40rpx auto","color":"#fff","borderRadius":"60rpx","textAlign":"center","background":"#174add","width":"50%","lineHeight":"80rpx","height":"80rpx"}' style="z-index: 11" @click="onUpdateTap()">新增</button>
			<view class="list" :style='{"padding":"60rpx 20rpx"}'>
				<view :style='{"width":"100%","margin":"0 0 16rpx","fontSize":"32rpx","color":"#000"}'>
					社区互动
				</view>
				<view style="position: relative" @tap="onDetailTap(item.id)" v-for="(item,index) in list " v-bind:key="index" :style='{"border":"0px solid #ddd","padding":"30rpx","margin":"0 0 20rpx","borderRadius":"20rpx","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between"}'>
