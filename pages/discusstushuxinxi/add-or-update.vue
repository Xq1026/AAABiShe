...
<view class="content">
	<view :style='{"width":"100%","padding":"24rpx","position":"relative","background":"#fff","height":"100%"}' v-if="showType==1">
		<form :style='{"width":"100%","padding":"48rpx 0 48rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"10rpx 20rpx","margin":"0","borderColor":"#1ccb96","borderRadius":"20rpx","borderWidth":"0px 0","background":"#f5f5f5","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"100%","lineHeight":"80rpx","fontSize":"30rpx","color":"#666","fontWeight":"500"}' class="title">评分</view>
		...
		methods: {
			...
			async onSubmitTap(subMitType=null) {
				...
				if(this.ruleForm.id){
					...
					oet = await this.$api.add(`discusstushuxinxi`, this.ruleForm);
				}
				uni.setStorageSync('discusstushuxinxiCleanType',true);
				let rs2 = await this.$api.list('discusstushuxinxi',{
					page: 1,
					limit: 10000,
				...
				let rs3 = await this.$api.info('tushuxinxi',this.ruleForm.refid)
				rs3.data.totalscore = rs2.data.list.length?Number((totalscore / rs2.data.list.length).toFixed(2)):0
				await this.$api.update('tushuxinxi',rs3.data)
				if(this.cross){
