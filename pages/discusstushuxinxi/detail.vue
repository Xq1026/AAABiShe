	...
	export default {
		...
		async onShow(options) {
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discusstushuxinxi', this.id);
		...
		methods: {
			...
			async init(type=1){
				...
				if(type==2){
					...
					await this.$api.update('discusstushuxinxi',this.detail)
				}
			},
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscusstushuxinxi',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			delClick(id){
				...
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscusstushuxinxi',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discusstushuxinxi',that.detail)
							setTimeout(()=>{
								that.mescroll.num = 1
