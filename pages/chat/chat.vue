...
<script>
	...
	export default {
		...
		onShow(){
			...
			this.init()
			if(this.timeChange()) {
				this.intelligenceSave("您好，有什么可以帮您？")
			}
		},
		onHide() {
			uni.closeSocket()
		...
		methods: {
			timeChange() {
				let commentList = JSON.parse(JSON.stringify(this.list)).reverse()
				if(!commentList.length) {
					return true
				}
				if(commentList[0].reply&&commentList[0].reply=="您好，有什么可以帮您？") {
					return false
				}
				let lastTime = new Date().getTime();
				const currentTime = new Date(commentList[0].addtime).getTime();
			...
			async intelligenceGet(ask) {
				this.ask = '';
				this.init();
				let res = await this.$api.baiduIdentify3('askai', {ask: ask})
				if (res.code == 0) {
			...
			async save(asks = null,type=1) {
				...
				let ask = asks?asks:JSON.parse(JSON.stringify(this.ask))
				this.ask = '';
