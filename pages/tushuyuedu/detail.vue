		...
		<view class="container" :style='{"minHeight":"100vh","width":"100%","padding":"0","position":"relative","background":"#fff","height":"auto"}'>
			...
			<view :style='{"alignContent":"flex-start","padding":"0","margin":"0 auto","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","height":"auto","zIndex":"9"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"0 24rpx","margin":"40rpx auto","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"20"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","boxShadow":"none","margin":"0 10rpx 10rpx 0","color":"#174add","borderRadius":"8rpx","background":"#d3e5ff","width":"auto","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx"}' v-if="userid&&isAuth('tushuyuedu','归还')" @tap="onAcrossTap('tushuguihai','','','','')" class="cu-btn bg-brown round shadow-blur" >归还</button>
					<button :style='{"border":"0","padding":"0 20rpx","boxShadow":"none","margin":"0 10rpx 10rpx 0","color":"#174add","borderRadius":"8rpx","background":"#d3e5ff","width":"auto","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('tushuyuedu','归还')" @tap="onAcrossTap('tushuguihai','','','','')" class="cu-btn bg-brown round shadow-blur" >归还</button>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		async onShow(options) {
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('tushuyuedu', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','tushuyuedu');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','tushuyuedu');
			...
			async init(type=1){
				...
				let res = await this.$api.info('tushuyuedu', this.id);
				let reg=new RegExp('http://localhost:8080/xiaoyuan1rxvwvck/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('tushuyuedu',this.detail)
				}
			},
