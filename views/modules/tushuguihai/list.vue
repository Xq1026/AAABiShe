...
<template>
	<div class="main-content" :style='{"padding":"22px 28px"}'>
				...
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('tushuguihai','超时')" @click="jieyuechaoshiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								超时
							</el-button>
							<el-button class="btn8" v-if="isAuth('tushuguihai','退还')" @click="yajintuihaiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								退还
							</el-button>
		...
		<jieyuechaoshi-cross-add-or-update v-if="jieyuechaoshiCrossAddOrUpdateFlag" :parent="this" ref="jieyuechaoshiCrossaddOrUpdate"></jieyuechaoshi-cross-add-or-update>
		<yajintuihai-cross-add-or-update v-if="yajintuihaiCrossAddOrUpdateFlag" :parent="this" ref="yajintuihaiCrossaddOrUpdate"></yajintuihai-cross-add-or-update>
...
<script>
	...
	import jieyuechaoshiCrossAddOrUpdate from "../jieyuechaoshi/add-or-update";
	import yajintuihaiCrossAddOrUpdate from "../yajintuihai/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				jieyuechaoshiCrossAddOrUpdateFlag: false,
				yajintuihaiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			jieyuechaoshiCrossAddOrUpdate,
			yajintuihaiCrossAddOrUpdate,
		},
		methods: {
			refreshClick(){
				this.searchForm = {}
			...
			jieyuechaoshiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.jieyuechaoshiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jieyuechaoshiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jieyuechaoshiCrossaddOrUpdate.init(row.id,type);
				});
			},
			yajintuihaiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				let username = this.$storage.get("adminName")
				if(row.yonghuzhanghao != username) {
					this.$message.error('无操作权限');
					return false
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yajintuihaiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','tushuguihai');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yajintuihaiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yajintuihaiCrossaddOrUpdate.init(row.id,type);
				});
			},
			init () {
			},
