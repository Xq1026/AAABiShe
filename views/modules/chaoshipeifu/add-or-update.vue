		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="可借天数" prop="kejietianshu" >
					<el-input v-model.number="ruleForm.kejietianshu" placeholder="可借天数" clearable  :readonly="ro.kejietianshu"></el-input>
				</el-form-item>
				...
				<el-form-item class="date" v-if="type!='info'" label="赔付时间" prop="peifushijian" >
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.peifushijian" 
						type="datetime"
						:readonly="ro.peifushijian"
						placeholder="赔付时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.peifushijian" label="赔付时间" prop="peifushijian" >
					<el-input v-model="ruleForm.peifushijian" placeholder="赔付时间" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="借阅时长" prop="jieyueshizhang" >
					<el-input-number v-model="ruleForm.jieyueshizhang" placeholder="借阅时长" :disabled="ro.jieyueshizhang"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="借阅时长" prop="jieyueshizhang" >
					<el-input v-model="ruleForm.jieyueshizhang" placeholder="借阅时长" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="超出时长" prop="chaochushizhang" >
					<el-input v-model.number="ruleForm.chaochushizhang" placeholder="超出时长" clearable  :readonly="ro.chaochushizhang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="超出时长" prop="chaochushizhang" >
					<el-input v-model="ruleForm.chaochushizhang" placeholder="超出时长" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="超时金额" prop="chaoshijine" >
					<el-input-number v-model="ruleForm.chaoshijine" placeholder="超时金额" :disabled="ro.chaoshijine"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="超时金额" prop="chaoshijine" >
					<el-input v-model="ruleForm.chaoshijine" placeholder="超时金额" readonly></el-input>
				</el-form-item>
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='peifushijian'){
							this.ruleForm.peifushijian = obj[o];
							this.ro.peifushijian = true;
							continue;
						}
						if(o=='jieyueshizhang'){
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `chaoshipeifu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										duration: 1500,
										onClose: () => {
											if(this.isAuth('chaoshipeifu','支付')&&this.type=='cross') {
												this.$confirm('是否跳转支付？').then(_ => {
													this.parent.showFlag = true;
													this.parent.addOrUpdateFlag = false;
													this.parent.chaoshipeifuCrossAddOrUpdateFlag = false;
													this.$router.push('/chaoshipeifu')
												}).catch(_ => {
													this.parent.showFlag = true;
													this.parent.addOrUpdateFlag = false;
													this.parent.chaoshipeifuCrossAddOrUpdateFlag = false;
