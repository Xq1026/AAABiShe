...
<template>
	<div class="main-content" :style='{"padding":"22px 28px"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0 0 0px","borderColor":"#fff","borderRadius":"32px 32px 0 0","background":"#f1f7fd","borderWidth":"1px 1px 0 1px","width":"100%","borderStyle":"solid"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0 30px 30px 30px","margin":"0","borderRadius":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%"}'>
					...
					<el-button class="add" v-if="isAuth('jieyuechaoshi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('jieyuechaoshi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":" 0px 15px 20px","boxShadow":"none","borderColor":"#fff","borderRadius":"0","background":"#f1f7fd","borderWidth":"0 1px 1px 1px","width":"100%","borderStyle":"solid"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true'
												prop="chaochushizhang"
						label="超出时长">
						<template slot-scope="scope">
							{{scope.row.chaochushizhang}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'
												prop="fajinbili"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jieyuechaoshi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('jieyuechaoshi','赔付')" @click="chaoshipeifuCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								赔付
							</el-button>
							<el-button class="edit" v-if=" isAuth('jieyuechaoshi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jieyuechaoshi','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<chaoshipeifu-cross-add-or-update v-if="chaoshipeifuCrossAddOrUpdateFlag" :parent="this" ref="chaoshipeifuCrossaddOrUpdate"></chaoshipeifu-cross-add-or-update>
...
<script>
	...
	import chaoshipeifuCrossAddOrUpdate from "../chaoshipeifu/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				chaoshipeifuCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			chaoshipeifuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.chaoshipeifuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jieyuechaoshi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.chaoshipeifuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.chaoshipeifuCrossaddOrUpdate.init(row.id,type);
				});
			},
			init () {
			},
