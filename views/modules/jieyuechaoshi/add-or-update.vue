	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="罚金比例" prop="fajinbili" >
					<el-input-number v-model="ruleForm.fajinbili" placeholder="罚金比例" :disabled="ro.fajinbili"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="罚金比例" prop="fajinbili" >
					<el-input v-model="ruleForm.fajinbili" placeholder="罚金比例" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'" label="超时金额" prop="chaoshijine" >
					<el-input v-model="chaoshijine" placeholder="超时金额" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.chaoshijine" label="超时金额" prop="chaoshijine" >
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					this.ruleForm.fajinbili = Number('0.05'); 				}
				// 获取用户信息
				this.$http({
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jieyuechaoshi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jieyuechaoshiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
