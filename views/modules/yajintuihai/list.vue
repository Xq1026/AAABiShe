		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0 0 0px","borderColor":"#fff","borderRadius":"32px 32px 0 0","background":"#f1f7fd","borderWidth":"1px 1px 0 1px","width":"100%","borderStyle":"solid"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0 30px 30px 30px","margin":"0","borderRadius":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%"}'>
					<el-button class="add" v-if="isAuth('yajintuihai','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('yajintuihai','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yajintuihai','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
				...
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('yajintuihai','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yajintuihai','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('yajintuihai','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('yajintuihai','删除')" type="primary" @click="deleteHandler(scope.row.id)">
...
<script>
	...
	export default {
		...
		methods: {
			...
			payHandler(row){
				this.$storage.set('paytable','yajintuihai');
