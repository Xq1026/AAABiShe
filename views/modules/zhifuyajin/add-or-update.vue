	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="支付时间" prop="zhifushijian" >
					<el-date-picker
				...
				<el-form-item class="input" v-else-if="ruleForm.zhifushijian" label="支付时间" prop="zhifushijian" >
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `zhifuyajin/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										duration: 1500,
										onClose: () => {
											if(this.isAuth('zhifuyajin','支付')&&this.type=='cross') {
												this.$confirm('是否跳转支付？').then(_ => {
													this.parent.showFlag = true;
													this.parent.addOrUpdateFlag = false;
													this.parent.zhifuyajinCrossAddOrUpdateFlag = false;
													this.$router.push('/zhifuyajin')
												}).catch(_ => {
													this.parent.showFlag = true;
													this.parent.addOrUpdateFlag = false;
													this.parent.zhifuyajinCrossAddOrUpdateFlag = false;
