...
<template>
	<div class="main-content" :style='{"padding":"22px 28px"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0 0 0px","borderColor":"#fff","borderRadius":"32px 32px 0 0","background":"#f1f7fd","borderWidth":"1px 1px 0 1px","width":"100%","borderStyle":"solid"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"30px","borderRadius":"8px 8px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative"}' >
					<div :style='{"margin":"0 1% 10px 0","display":"flex"}' class="select">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">是否支付</label>
				...
				<el-row class="actions" :style='{"padding":"0 30px 30px 30px","margin":"0","borderRadius":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%"}'>
					<el-button class="add" v-if="isAuth('zhifuyajin','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('zhifuyajin','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('zhifuyajin','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
				...
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('zhifuyajin','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zhifuyajin','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('zhifuyajin','阅读')" @click="tushuyueduCrossAddOrUpdateHandler(scope.row,'cross','否','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								阅读
							</el-button>
							<el-button class="edit" v-if=" isAuth('zhifuyajin','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('zhifuyajin','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<tushuyuedu-cross-add-or-update v-if="tushuyueduCrossAddOrUpdateFlag" :parent="this" ref="tushuyueduCrossaddOrUpdate"></tushuyuedu-cross-add-or-update>
...
<script>
	...
	import tushuyueduCrossAddOrUpdate from "../tushuyuedu/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				tushuyueduCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			tushuyueduCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.tushuyueduCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','zhifuyajin');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.tushuyueduCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.tushuyueduCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				this.$storage.set('paytable','zhifuyajin');
